...
#
# Version:    2.0.2023083107081693490905
# Build Hash: 35f0b5ce5bf71c1ff29a67bb9561f6b2ed48a108
#
# ------------------------------------------------------------------------
...
if [[ "${1}" ]]; then
  ...
  readonly DEPLOY_URL="https://storage.googleapis.com/cloudsapdeploy/deploymentmanager/202308310708/dm-templates"
fi
...
metrics::send_metric() {(  #Exits will only exit the sub-shell.
    ...
    RUNNING|STARTED|STOPPED|CONFIGURED|MISCONFIGURED|INSTALLED|UNINSTALLED)
        user_agent="sap-core-eng/accelerator-template/2.0.2023083107081693490905/${VM_IMAGE}/${status}"
        ;;
    ERROR)
        metrics::validate "${error_id}" "'ERROR' statuses require the error message (-e) argument."
        user_agent="sap-core-eng/accelerator-template/2.0.2023083107081693490905/${VM_IMAGE}/${status}/${error_id}-${template_id}"
        ;;
    UPDATED)
        metrics::validate "${updated_version}" "'UPDATED' statuses require the updated version (-u) argument."
        user_agent="sap-core-eng/accelerator-template/2.0.2023083107081693490905/${VM_IMAGE}/${status}/${updated_version}"
        ;;
    ACTION)
        metrics::validate "${action_id}" "'ACTION' statuses require the action id (-c) argument."
        user_agent="sap-core-eng/accelerator-template/2.0.2023083107081693490905/${VM_IMAGE}/${status}/${action_id}"
        ;;
    TEMPLATEID)
        user_agent="sap-core-eng/accelerator-template/2.0.2023083107081693490905/${VM_IMAGE}/ACTION/${template_id}"
        ;;
    *)
